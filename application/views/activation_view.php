...
<html class="no-js linen" lang="en" xmlns="http://www.w3.org/1999/html"><!--<![endif]-->
...
<head>
    ...
    <!-- Login pages styles -->
    <link rel="stylesheet" media="screen" href="<?php echo base_url();?>assets/css/reg_succ.css?v=1">
...
<div id="container" class="three-columns ten-columns-mobile-landscape ten-columns-tablet ten-columns-mobile align-center">


    <div class="carbon with-padding">
        <div class="boxed white-bg" id="message-block">
            <div class="align-center twelve-columns">

                <?php
                if($process)
                {
                    echo "<h2>Votre compte est maintenant activé.</h2>

                    <h4>Cliquez sur Connexion pour commencer.</h4>";
                }
                else
                {
                    echo "<h2>Erreur lors de l'activation de votre compte.</h2>
                    <h4 class='red-gradient with-padding'>Votre compte est peut-être déjà activé.</h4>
                    <h4>Veuillez réessayer plus tard, si le problème persiste, contactez l'un de nos administrateurs.</h4>";
                }
                ?>

                <a href="<?php echo base_url('login'); ?>" class="button blue-gradient glossy align-center big margin-top">Connexion</a>
            </div>
...
<script>
    ...
    $(document).ready(function()
    {
         ...
         * This script will enable effects for the login page
         ...
         container.css('top', Math.max(top, top + Math.round($.template.viewportHeight-container.outerHeight())/2)+'px');

         //container.css('left', Math.max(left, left + Math.round($.template.viewportWidth-container.outerWidth())/2)+'px');
         */
