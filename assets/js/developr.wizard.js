...
*/
	...
	function _endStepChange(form, fieldset, step, previous, newStep)
	{
		...
		event.wizard = {
			current: previous[0],
			target: fieldset[0],
			forward: (fieldset.prevAll().filter(previous[0]).length > 0)
		};
	...
	$.fn.wizard.defaults = {
		 ...
		 * @var string
		 */
		controlPrev: '<button type="button" class="button glossy mid-margin-right wizard-prev float-left"><span class="button-icon anthracite-gradient"><span class="icon-backward"></span></span>Retour</button>',

		/**
		 * Next button markup - must use class 'wizard-next'
		 * @var string
		 */
		controlNext: '<button type="button" class="button glossy mid-margin-right wizard-next float-right">Suivant<span class="button-icon right-side"><span class="icon-forward"></span></span></button>',
